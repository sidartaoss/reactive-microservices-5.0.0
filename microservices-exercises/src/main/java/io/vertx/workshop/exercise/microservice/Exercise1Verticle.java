...
/**
 * 
 * This verticle exposes a Http endpoint service. You need to publish it in
...
public class Exercise1Verticle extends AbstractVerticle {
    ...
    public void start(Future<Void> future) throws Exception {
        ...
        // As we want to complete the deployment when the service is exposed (asynchronous operation), we
        // use a `Future`argument to indicate when the deployment is completed.
        // This allows deploying the other verticle after the deployment completion of this one.
        ...
        router.get("/greetings")
                .handler(rc -> rc.response().end("Hello World"));
        router.get("/greetings/:name")
                .handler(rc -> rc.response().end("Hello " + rc.pathParam("name")));
